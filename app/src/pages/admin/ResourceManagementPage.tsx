          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-4">
                  {/* 图标预览 */}
                  <div className="w-16 h-16 rounded-xl border-2 border-gray-200 flex items-center justify-center bg-gray-50 flex-shrink-0 overflow-hidden">
                    {formLinkIcon ? (
                      <img
                        src={formLinkIcon}
                        alt="预览"
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {/* 直接上传按钮 */}
                    <label className="inline-flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 cursor-pointer text-sm text-gray-600 transition-colors">
                  ...
                  >
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="搜索图标库..."
                        className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 bg-white"
                      />
                    </div>
                    {iconLibrary.length === 0 ? (
                      <div className="text-center py-4 text-gray-400 text-sm">
                        <Image className="w-6 h-6 mx-auto mb-1 opacity-30" />
                        图标库为空，请先到「数据管理 → 图标库」上传图标
                      </div>
                    ) : (
                      <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-2 max-h-48 overflow-y-auto p-1">
                        {iconLibrary
                          .filter(icon => !iconPickerSearch || icon.name.toLowerCase().includes(iconPickerSearch.toLowerCase()))
                          .map((icon) => (
                          <button
                            key={icon.id}
                            type="button"
                            onClick={() => {
                              setFormLinkIcon(icon.dataUrl)
                              setShowIconPicker(false)
                            }}
                            className={`p-2 rounded-lg border transition-all hover:shadow-sm ${
                              formLinkIcon === icon.dataUrl
                ...
                <p className="text-xs text-gray-400">支持 JPG、PNG 格式，大小不超过 2MB</p>
              </div>
            </div>
